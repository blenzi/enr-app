...
regions = st.sidebar.multiselect('Régions', liste_regions, key='regions', on_change=limit_choices, args=('regions', ))

reg = regions or slice(None)
liste_departements = load_zones().set_index(['TypeZone', 'Region']).loc[('Départements', reg), 'Zone'].to_list()
departements = st.sidebar.multiselect('Départements', liste_departements, key='departements',
                                      on_change=limit_choices, args=('departements', ))
...
if zone:
    ...
    st.write("Veuillez sélectionner des régions ou départements, ainsi que l'année, l'indicateur et les filières")
